...
public class HealthBarRenderer {
    ...
    public static MovingObjectPosition getMouseOverExtended(float dist) {
        ...
        AxisAlignedBB theViewBoundingBox =
                AxisAlignedBB.getBoundingBox(theRenderViewEntity.posX - 0.5D, theRenderViewEntity.posY - 0.0D, theRenderViewEntity.posZ - 0.5D, theRenderViewEntity.posX + 0.5D,
    ...
    private static float doRenderHealthBar(EntityLivingBase passedEntity,
                                           float partialTicks,
                                           Entity viewPoint,
                                           ...
                                           float pastTranslate,
                                           Minecraft mc) {
        float brightness = getBrightness(passedEntity);
        ...
        GL11.glTranslatef((float) (x - RenderManager.renderPosX), (float) (y - RenderManager.renderPosY + passedEntity.height + NeatConfig.heightAbove),
        ...
        double scale = computeScale(interpDistance, sr, entity);
        ...
        if (NeatConfig.showMaxHP) {
            mc.fontRenderer.drawString(maxHpStr, (int) (size / (s * s1) * 2) - 2 - mc.fontRenderer.getStringWidth(maxHpStr), h,
                                       NeatConfig.darknessAdjustmentText ? argbText : fullBrightText);
        }
        if (NeatConfig.showPercentage) {
            mc.fontRenderer.drawString(percStr, (int) (size / (s * s1)) - mc.fontRenderer.getStringWidth(percStr) / 2, h,
                                       NeatConfig.darknessAdjustmentText ? argbText : fullBrightText);
        }
    ...
    private static float getBrightness(EntityLivingBase passedEntity) {
    ...
    private static double computeScale(double interpDistance, ScaledResolution sr, EntityLivingBase entity) {
            ...
            for (Entity entity : entities) {
                ...
                if (isHbmLoaded()) {
                    if (entity != null && (entity instanceof EntityLiving || entity instanceof EntityPlayer) && entity != mc.thePlayer &&
                        entity.isInRangeToRender3d(renderingVector.xCoord, renderingVector.yCoord, renderingVector.zCoord) && entity.isEntityAlive()) {
                        if (NeatConfig.HbmEnemyHUD) {
                            if (ArmorFSB.hasFSBArmor(mc.thePlayer)) {
                                ItemStack plate = mc.thePlayer.inventory.armorInventory[2];
                                ArmorFSB chestplate = (ArmorFSB) plate.getItem();
                        ...
                        /*&& (entity.ignoreFrustumCheck || frustrum.isBoundingBoxInFrustum(entity.boundingBox))*/ &&
                         entity.isEntityAlive()) //isBoundingBoxInFrustum turned out to be the cause of an issue where bars weren't rendering at certain angles, and it seems to be completely unnecessary since bars don't render after a certain distance, and don't render behind blocks
                {
                    renderHealthBar((EntityLivingBase) entity, event.partialTicks, cameraEntity);
                }
            }
    ...
    public void renderHealthBar(EntityLivingBase passedEntity, float partialTicks, Entity viewPoint) {
        ...
        while (true) {
            ...
            try {
                pastTranslate = doRenderHealthBar(passedEntity, partialTicks, viewPoint, entity, sr, pastTranslate, mc);
            } finally {
                GL11.glPopAttrib();
            }
